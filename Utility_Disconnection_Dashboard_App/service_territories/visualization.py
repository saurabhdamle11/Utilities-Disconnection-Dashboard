...
from .config import (
    ...
    US_CENCUS_PATH, NO_DATA_PATH, WRITEUP_PATH, COMBINED_PROVIDERS, STATE_PROVIDERS,_DATA_PATH
)
...
# sesrvice territories visualization

# 'combined_providers.csv': correlates utilities in disconnection data with a geographic area
provider_lookup_table = pd.read_csv(COMBINED_PROVIDERS, dtype={'electric ID':str, 'gas ID': str})

# 'state_providers.csv': eia providers after being clipped with state borders
state_providers_df=pd.read_csv(STATE_PROVIDERS)

# 'gz_2020_us_040_00_20m.json': US state borders geojson with polygons, state names and numbers
geo_providers_df = pd.merge(state_providers_df[['NAME', 'ID_STATE', 'GEO_STATE_ID', 'GEO_STATE_NAME']],provider_lookup_table, how= 'right', left_on= ('NAME'), right_on=('EIA_Name') )
geo_providers_df[geo_providers_df['GEO_STATE_NAME']=="Indiana"]

f = open(_DATA_PATH+'gz_2010_us_040_00_20m.json')
data = json.load(f)
state_names = [x['properties']['NAME'] for x in data['features']]
state_nums = [x['properties']['STATE'] for x in data['features']]
state_num_name_dict = dict(zip(state_nums, state_names))
state_num_name_list =list(zip(state_nums, state_names))


def build_df(ejl_data=_DATA_PATH+'hard_fix_feb_7.csv',
             provider_data=geo_providers_df,
             id_field = 'ID_STATE',
             state_field = 'GEO_STATE_NAME',
             year = None,
             month = None,
             state = None):
    df = pd.read_csv(ejl_data)
    df = pd.merge(df, provider_data, how= 'left', on= ('Utility Name') )
    df['ID']=df[id_field]
    ...
    if "All" in month or len(month) == 0:
        month = list(MONTHS)
    if "All" in state or len(state) == 0:
        state = list(STATE_CODE_TO_NAME.values())
    

    df = df.loc[
        (df["Year"].isin(year)) &
        (df["Month"].isin(month))&
        (df[state_field].isin(state))
    ]

    df=df[['Utility Name','Total Disconnections', 'ID', 'GEO_STATE_NAME']]
    df = df[df['ID'].notna()]
    df = df[df['Total Disconnections'].notna()]
    df = df.groupby(['ID', 'Utility Name', 'GEO_STATE_NAME'])['Total Disconnections'].mean().reset_index(name='Avg Disconnections')

    return df

def plot_map(year = None, month =None, state=None):
    
    df = build_df(year=year, month=month, state = state)
    if state is not None and state !='All':
        state_id = state_nums[state_names.index(state)]
        f = open(os.path.join(_DATA_PATH+'state_providers', state_id+"_providers.geojson"))
    else:
        f=open(_DATA_PATH+'state_providers.geojson')

    elec_geojson = json.load(f)
    fig = px.choropleth_mapbox(df, geojson=elec_geojson, featureidkey= 'properties.ID_STATE', locations='ID', color='Avg Disconnections',
                        color_continuous_scale="PuRd",
                        range_color=(0, df['Avg Disconnections'].max()),
                        mapbox_style="carto-positron",
                        zoom=3, center = {"lat": 37.0902, "lon": -95.7129},
                        opacity=1,
                        hover_name='Utility Name',
                        hover_data = {'GEO_STATE_NAME': True, 'Avg Disconnections': True, 'ID': False}
                        )
    fig.update_layout(margin={"r":0,"t":0,"l":0,"b":0})
    fig.update_coloraxes(
    colorbar_title_text="Average Disconnections per provider",
